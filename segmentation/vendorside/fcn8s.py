...
class FCN8s(nn.Module):
    ...
    def adjust_learning_rate(self, args, optimizer, i):
        # changed from 50k to 20k since using higher batch size
        optimizer.param_groups[0]['lr'] = args.lr * (0.1**(int(i/20000)))
        if len(optimizer.param_groups) > 1:
            optimizer.param_groups[1]['lr'] = args.lr * (0.1**(int(i/20000))) * 2
